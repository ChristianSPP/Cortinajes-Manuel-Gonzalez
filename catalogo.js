...
function actualizarEnlacePrecios(pdfFile) {
  const enlace = document.getElementById('enlace-precios');
  const mensaje = document.getElementById('mensaje-precios');

  if (!enlace || !mensaje) return;

  if (pdfFile) {
    enlace.href = pdfFile;
    enlace.removeAttribute('aria-disabled');
    enlace.classList.remove('pointer-events-none');
    mensaje.classList.add('hidden');
  } else {
    enlace.href = '#';
    enlace.setAttribute('aria-disabled', 'true');
    enlace.classList.add('pointer-events-none');
